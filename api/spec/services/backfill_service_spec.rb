...
RSpec.describe BackfillService do
  ...
  it 'writes one snapshot per calendar day, carrying prices forward when markets are closed' do
    allow(Date).to receive(:today).and_return(Date.new(2026, 2, 15))

    result = described_class.call(client: fake_client)
    expect(result[:snapshots_written]).to eq(3)
    ...
    weekend = PortfolioSnapshot[portfolio_id: portfolio.id, date: Date.new(2026, 2, 14)]
    expect(weekend.total_value_pln).to eq(BigDecimal('437.8'))
  end
end
